...
module.exports = exports = function mongooseIntl(schema, options) {
	if (schema.options.mongooseIntl) {
		return;
	}
	if (
		!options ||
	...
	schema.eachPath(function (path, schemaType) {
		...
		schema
			...
			.get(function () {
				...
				if (process.env.NEXT_PUBLIC_APP_NAME === "Laptop Outlet Admin")
					return langSubDoc;
				...
				if (langSubDoc.hasOwnProperty("es")) {
					return langSubDoc["es"];
				}
	...
	schema.on("init", function (model) {
		// no actions are required if the global method is already defined
